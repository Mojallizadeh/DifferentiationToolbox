...
function [u,calculation_time] = I_GHDD(f_k,h,L,reset_flag,output_order,x0)
% Arbitrary differentiator order  
    ...
    n=output_order;
        ...
        alpha12=-1/2;alpha13=1/3;alpha23=-1;
        ...
        if bk<-a4
            ...
            z3=h*(-lambda3*L)+z3;
            z2=h*(-lambda2*L^(3/4)*abs(s)^1 +z3  )+z2  -alpha23*h^2*lambda3*L;    
            z1=h*(-lambda1*L^(2/4)*abs(s)^2 +z2  )+z1   +h^2*z3/2    -alpha12*h^2*lambda2*L^(3/4)*abs(s)  -   alpha13*h^3*lambda3*L  ;     
        ...
        elseif bk>a4
            ...
            z3=z3+(bk)/h^n;
            z2=z2+h*z3    -           alpha23*bk;
            z1=z1+h*z2   +h^2*z3/2     -   alpha13*bk;
